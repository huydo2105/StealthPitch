...
export default function DealActionButtons({
    ...
    const canAcceptExit =
        room.proposed_price > 0 && !dealOutcome && (room.status === "funded" || room.status === "negotiating");
    const canCancel = room.status === "created" && !dealOutcome && isSeller;

    return (
        <div className="px-4 pb-4">
            {/* Accept / Exit (TEE-signed on-chain) */}
            {canAcceptExit && (
                <div className="flex gap-2 mb-3">
                    <button
                        onClick={onAccept}
                        disabled={loading || room.proposed_price < room.seller_threshold}
                        title={room.proposed_price < room.seller_threshold ? "Proposed price is below threshold" : "TEE signs acceptDeal on-chain"}
